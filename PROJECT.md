...
- **审核台**（/admin，P3）：轻量审核页，登录用 `ADMIN_PASSWORD`（请求头 `x-admin-token`，localStorage key `ai_opc_admin_token`）。可批量发布/丢弃雷达 draft 和周报 draft。API：`app/api/admin/review/route.ts`（GET 草稿列表）、`app/api/admin/publish/route.ts`（POST 发布/丢弃）。**需要 Vercel 环境变量 `SUPABASE_SERVICE_ROLE_KEY`**（radar_items 开了 RLS，anon 只读）和 `ADMIN_PASSWORD`；页面无公开导航入口，自行收藏网址
