...
- **审核台**（/admin，P3）：轻量审核页，登录用 `ADMIN_PASSWORD`（请求头 `x-admin-token`，localStorage key `ai_opc_admin_token`）。可批量发布/丢弃雷达 draft 和周报 draft、编辑后再发布（标题/摘要/点评/评分/分类/收录理由）、**已发布内容在线编辑与下架回炉**（下架即退回草稿区，published 不可直接删除），也可手动触发雷达抓取/周报生成（通过 GitHub workflow_dispatch）。API：`app/api/admin/review/route.ts`（GET 草稿+已发布+弃选列表）、`app/api/admin/publish/route.ts`（POST 发布/丢弃/下架）、`app/api/admin/edit/route.ts`（POST 编辑，draft+published 均可）、`app/api/admin/trigger/route.ts`（POST 触发工作流）。**需要 Vercel 环境变量**：`SUPABASE_SERVICE_ROLE_KEY`（radar_items 开了 RLS，anon 只读）、`ADMIN_PASSWORD`、`GITHUB_PAT`（GitHub classic PAT，repo scope，用于手动触发）；页面无公开导航入口，自行收藏网址
