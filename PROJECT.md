...
- **编辑视觉**: `docs/design/EDITORIAL-VISUAL-SYSTEM.md` 记录2026-09-16移动列表密度、五张栏目插画及报告头图规范；栏目组件 `components/category-hero.tsx`，WebP资产在 `public/editorial/`。
...
- **机会生产线**：`weekly-opportunities.yml` 每周三09:23北京时间及手动运行。近14天published雷达至少3信号聚类 → 联网发现含中国相关原文 → 安全读取 → 模型仅按已读摘录编号研究 → 至少3摘录/2页面/1直接证据与落地字段校验 → draft人工审核。源码与准则见 `docs/OPPORTUNITY-EDITORIAL-STANDARD.md`。未过门槛记录待补证/复核原因，不写公开机会；阻塞时任务失败并上传 `opportunity-delivery.json`。角色区分：机会库提供完整公共研究，方向探测器结合个人条件继续比较。现有发布版不自动改写。
