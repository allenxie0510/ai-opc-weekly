...
export function resolveImageUrl(src) {
  ...
  // 原图链接可能是 /pic/orig/media%2F...；pbs.twimg.com 不需要 orig/ 前缀。
  p = p.replace(/^orig\//, '');
...
function proxiedPreviewUrl(raw, sourceUrl) {
  if (!raw || /profile_images|emoji/i.test(raw)) return null;
  let absolute;
  try {
    absolute = new URL(unescapeXml(raw), sourceUrl).toString();
  } catch {
    return null;
  }
  const resolved = resolveImageUrl(absolute);
  // 视频文件本身不能交给图片代理；这里只保留图片或视频 poster/thumbnail。
  if (/\.(?:mp4|m3u8|mov|webm)(?:$|[?#])/i.test(resolved)) return null;
  return '/api/img-proxy?url=' + encodeURIComponent(resolved);
}

/**
 * 从 RSS item 或 Nitter HTML 推文块提取图片预览及视频静态封面。
 * 兼容 img[src]、video[poster]、media:thumbnail[url] 和 image 类型 media:content。
 */
export function extractMediaPreviews(fragment, sourceUrl = 'https://x.com') {
  const previews = [];
  const tags = fragment.match(/<(?:img|video|media:thumbnail|media:content)\b[^>]*>/gi) || [];
  for (const tag of tags) {
    const tagName = tag.match(/^<([\w:-]+)/)?.[1]?.toLowerCase();
    const type = attr(tag, 'type').toLowerCase();
    // media:content 指向 mp4 时跳过，等待同 item 中的 thumbnail/poster。
    if (tagName === 'media:content' && type.startsWith('video/')) continue;
    const raw = tagName === 'img'
      ? attr(tag, 'src')
      : tagName === 'video'
        ? attr(tag, 'poster')
        : attr(tag, 'url');
    const preview = proxiedPreviewUrl(raw, sourceUrl);
    if (preview) previews.push(preview);
  }
  return [...new Set(previews)].slice(0, 4);
}
 ...
 */
export function parseRSSFeed(xml, account, sourceUrl = 'https://x.com') {
  const tweets = [];
  ...
  while ((match = itemRegex.exec(xml)) !== null) {
    ...
    const previews = extractMediaPreviews(block, sourceUrl);
...
export function parseNitterTimelineHtml(html, account, sourceUrl = 'https://nitter.net') {
  ...
  for (let i = 0; i < starts.length; i++) {
    ...
    const mediaUrls = extractMediaPreviews(block, sourceUrl);
...
export async function fetchDocumentCurl(url, timeoutSec = 20) {
  ...
  return new Promise((resolve) => {
    execFile('curl', [
      ...
      const status = (m ? parseInt(m[1], 10) : 0) || -1;
      let body = m ? stdout.slice(0, m.index) : stdout;
      let headers = '';
      // -D - includes proxy/redirect headers. Only the final response governs Retry-After.
      while (/^HTTP\/\S+\s+\d{3}/.test(body)) {
        const end = body.search(/\r?\n\r?\n/);
        if (end < 0) break;
        headers = body.slice(0, end);
        body = body.slice(end).replace(/^\r?\n\r?\n/, '');
      }
      const retryAfterMs = parseRetryAfter(headers.match(/^retry-after:\s*(.+)$/im)?.[1]);
      const finalUrl = m?.[2] || url;
      if (err || status !== 200) return resolve({ ok: false, status, finalUrl, retryAfterMs, reason: err?.message?.split('\n')[0], transport: 'curl' });
...
export async function fetchDocumentNode(url, timeoutMs = 15000) {
  try {
    ...
    if (!res.ok) return { ok: false, status: res.status, finalUrl: res.url, retryAfterMs: parseRetryAfter(res.headers.get('retry-after')), transport: 'node-fetch' };
...
export function parseRetryAfter(value, now = Date.now()) {
  ...
  return Number.isFinite(date) ? Math.max(1000, date - now) : 120_000;
}

export function sourceKey(candidate) {
  const url = new URL(candidate.url);
  // These are aliases for the same service, not independent rate-limit budgets.
  if (url.hostname === 'rss.xcancel.com' || url.hostname === 'xcancel.com') return 'xcancel.com';
  return url.origin;
}

/** Wait only when every candidate is unavailable, until the first one may recover. */
export function sourceCooldownDelay(account, sources, sourceState, now = Date.now()) {
  const states = candidatesFor(account, sources).map(candidate => sourceState.get(sourceKey(candidate)));
  if (states.some(state => !state?.blocked && !(state?.retryAt > now))) return 0;
  const deadlines = states.filter(state => !state?.blocked && state?.retryAt > now).map(state => state.retryAt);
  return deadlines.length ? Math.min(...deadlines) - now : 0;
}
 ...
 *
 * @param {{ username: string, rss_url?: string | null }} acc
 * @param {{ timeoutSec?: number, debug?: boolean, sources?: any[], sourceState?: Map<string, any>, failureThreshold?: number, request?: typeof fetchDocument, now?: () => number }} [opts]
 */
export async function fetchAccountTweets(acc, {
  ...
  for (const c of candidatesFor(acc, activeSources)) {
    ...
    const state = sourceState?.get(key);
    if (state?.blocked || state?.retryAt > now()) {
      attempts.push(`${c.name} ${state?.blocked ? '本轮已熔断' : '限流冷却中'}`);
      continue;
    }

    const r = await request(c.url, { timeoutSec });
    if (r.ok) {
      const tweets = c.kind === 'html'
        ? parseNitterTimelineHtml(r.body, acc, r.finalUrl || c.url)
        : parseRSSFeed(r.body, acc, r.finalUrl || c.url);
      if (tweets.length > 0) {
        if (sourceState) sourceState.set(key, { failures: 0, blocked: false, retryAt: 0 });
      ...
      if (sourceState && r.status === 429) {
        sourceState.set(key, { failures: 0, blocked: false, retryAt: now() + (r.retryAfterMs || 120_000) });
        continue;
      }
      const systemic = r.status <= 0 || r.status === 403 || r.status >= 500;
      if (sourceState && systemic) {
        const failures = (state?.failures || 0) + 1;
        sourceState.set(key, { failures, blocked: failures >= failureThreshold });
      }
    }
  }
