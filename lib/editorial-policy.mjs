...
function validAnswer(key, field) {
  const length = normalize(field?.answer).length;
  return length >= (key === 'payer' ? 2 : field?.basis === 'unknown' ? 3 : 4) && length <= 360;
}
...
export function publishableBrief(brief) {
  return !!brief && brief.version === 1 && publicSourceUrl(brief.source_url)
    ...
    && Object.keys(QUESTION_LABELS).every(key => validAnswer(key, brief.answers?.[key]));
}
...
export function validateEditorialBrief(raw, material) {
  ...
  for (const key of Object.keys(QUESTION_LABELS)) {
    const field = raw.answers?.[key];
    const answer = normalize(field?.answer), quote = normalize(field?.quote);
    if (!validAnswer(key, field) || !['source', 'inference', 'unknown'].includes(field?.basis)) return { ok: false, reason: `missing-answer-${key}` };
...
回答不超过360字符；付费对象可为商家、开发者等明确短称，未披露可直接写未披露并标unknown；其余回答至少4字符。全部引用合计不超过480字符。problem、ai_role、evidence必须有源文支撑；缺少就拒绝，不用常识补齐。
