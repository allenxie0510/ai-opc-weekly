...
export function publishableBrief(brief) {
  return !!brief && brief.version === 1 && publicSourceUrl(brief.source_url)
    ...
    && Object.keys(QUESTION_LABELS).every(key => normalize(brief.answers?.[key]?.answer).length >= 4);
}
...
export function validateEditorialBrief(raw, material) {
  ...
  for (const key of Object.keys(QUESTION_LABELS)) {
    ...
    if (answer.length < 4 || answer.length > 360 || !['source', 'inference', 'unknown'].includes(field?.basis)) return { ok: false, reason: `missing-answer-${key}` };
...
六个回答各4–360字符，全部引用合计不超过480字符。problem、ai_role、evidence必须有源文支撑；缺少就拒绝，不用常识补齐。
