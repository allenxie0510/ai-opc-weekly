  ...
  return (
    <div className="pr-supply-grid">
      {offers.slice(0, 5).map((offer) => <article key={offer.id} className="pr-supply-card">
        ...
        <a href={offer.url} target="_blank" rel="noreferrer nofollow" onClick={() => trackProductRadarEvent('open_supply_offer', { slug, offerId: offer.id })}>去 1688 公开搜索 <LineIcon name="external-link" /></a>
      </article>)}
    </div>
  );
}
