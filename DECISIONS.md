...
- **结论**：雷达不是泛 AI 新闻。优先个人/2–5 人团队的真实产品、收入、客户、定价、获客和构建复盘；大公司只作为可迁移的背景信号。
- **来源层**：Founder-first 最多 32 条入模，Enabler 最多 16 条，Context 最多 6 条；单源另有限额。新增 BetaList AI 与 Reddit r/SideProject，均先在 Actions runner 实测真实 feed。
- **选取层**：删除“恰好 5–10 条”，改为允许 0 条、最终每天最多 6 条。GLM 生成五维 fit，代码要求 OPC 相关性≥4、可行动性≥3、证据≥3、单人可行性≥3、加权总分≥70。
- **真实性层**：`source_url` 必须存在于当轮素材，且模型必须提供逐字存在于对应标题/摘要中的证据锚点，source_name 由代码回填；同一来源最多 2 条。大公司每天最多 1 条，且迁移性≥4并给出目标用户、动作和 30 天验证方法。
- **踩过的坑**：抓取 128 条后按 `fetched_at desc` 只取 40 条，会让后抓/高频媒体占满 prompt；强制凑数会诱导模型把融资、模型发布、CEO 观点牵强包装成 OPC 机会。只改 prompt 不足以防复发，来源配额和出模门槛必须由代码执行。
