/**
 * 种子脚本 — 将 W27 周报数据写入 Supabase
 * 用法：
 *   NEXT_PUBLIC_SUPABASE_URL=xxx NEXT_PUBLIC_SUPABASE_ANON_KEY=xxx npx tsx scripts/seed-w27.ts
 *   或使用 service_role key:
 *   NEXT_PUBLIC_SUPABASE_URL=xxx SUPABASE_SERVICE_ROLE_KEY=xxx npx tsx scripts/seed-w27.ts
 */
...
async function main() {
  ...
  if (existing) {
    ...
    // Delete existing news_items for this issue
    const { error: delErr } = await supabase
      .from('news_items')
      .delete()
      .eq('weekly_issue_id', issueId);
    if (delErr) {
      console.error('❌ 删除旧 news_items 失败:', delErr.message);
      process.exit(1);
    }
    ...
    if (issueErr) {
      console.error('❌ 创建 weekly_issue 失败:', issueErr.message);
      process.exit(1);
    }
    issueId = newIssue.id;
  ...
  if (insertErr) {
    console.error('❌ 插入 news_items 失败:', insertErr.message);
    process.exit(1);
  }

  console.log(`✅ 成功写入 ${inserted?.length || 0} 条 news_items\n`);

  // 3. Verify
  const { data: verify, error: verifyErr } = await supabase
    .from('news_items')
    .select('id, title, category, rank')
    .eq('weekly_issue_id', issueId)
