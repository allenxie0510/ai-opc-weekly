...
六问答案必须放在 editorial_brief.answers 内，不能省略。answer用中文，4–360字符；payer写清标签，例如潜在付费对象：开发者。basis只能是source/inference/unknown；problem、ai_role、evidence必须是source并选择能支撑回答的本URL片段编号。其余未知写尚未披露，basis必须为unknown且quote_id为空；只有具体推断才标inference，不能把未披露标成推断。不要输出任何quote文字，只输出本URL真实的quote_id，由程序填回原文。
