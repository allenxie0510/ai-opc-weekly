...
test('真实 PostgreSQL 触发器：短中文答案规范化后雷达、周报均可入库及发布；空答案仍拒绝', async () => {
  ...
  try {
    ...
    await db.exec(sql);
    const fullReport = validateReportPayload(reportPayload(), testMaterial, 'postgres-test').item.editorial_brief;
    const reportRow = await db.query('insert into public.news_items(editorial_brief) values ($1) returning editorial_brief', [fullReport]);
    assert.equal(reportRow.rows[0].editorial_brief.weekly_report.analysis.customer, fullReport.weekly_report.analysis.customer);
    assert.equal(reportRow.rows[0].editorial_brief.weekly_report.version, 1);
