...
≤768px时插画采用覆盖头图内层的绝对定位背景层，`object-fit: contain`、右对齐，允许延伸到文字下方。保留原文字网格、坐标与留白；图片不再受右侧窄列裁切。移动图片sizes同步为100vw，桌面沿用原布局。检查320/390/768/1440px × 五栏目，核对完整显示、文字位置与横向溢出。
